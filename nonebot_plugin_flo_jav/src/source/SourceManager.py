...
class SourceManager:
    ...
    def __init__(self,
        ...
        self.proxy = proxy
        self.image_dir: Path = Path(data_dir) / "resources" / "images"
        if not self.image_dir.exists():
            self.image_dir.mkdir(parents=True, exist_ok=True)

        self.scraper_manager: ScraperManager = scraper
    ...
    def set_source_cookie(self, source_name: str, cookie: str) -> bool:
        ...
        if source_name.lower() in self.sources.keys():
            actual_name = source_name.lower()
            target_source = self.sources.get(actual_name)
    ...
    def get_info_from_any_source(self, avid: str) -> Optional[AVInfo]:
        """
        遍历所有源获取信息
        返回: AVInfo
        """
    ...
    def get_info_from_source(self, avid: str, source_str: str) -> Optional[AVInfo]:
        """
        从指定源获取信息
        ...
        # 查找对应的下载器（不区分大小写）
        source = None
        for name, possible_source in self.sources.items():
    ...
    def save_all_resources(self, info: AVInfo) -> None:
        """
        ...
        # 下载封面
        avid = info.get_avid()
        source = self.sources.get(info.get_source())
        image_url = info.get_image_url()
        if image_url:
            logger.info(f"封面下载地址: {image_url}")
            cover_path = self.image_dir / info.get_avid()
            if not source.download_file(image_url, str(cover_path)):
